...
namespace ATS.Data
{
    ...
    public class ATSDbContext: DbContext
    {
        ...
        public ATSDbContext(IWorkContext workContext)
            : base("ATS")
        {
        ...
        public override int SaveChanges()
        {
            ...
            var entityList = ChangeTracker.Entries().Where(p => p.State == EntityState.Added || p.State == EntityState.Modified);
            foreach (var entity in entityList)
            {
                DBAudit audit = auditFactory.GetAudit(entity, _workContext.UserId, _workContext.Role);
                if (audit.TableName == "LogExceptions" || audit.NewData.Count() == 0)
                    continue;
    ...
    public class AuditTrailFactory
    {
        ...
        public DBAudit GetAudit(DbEntityEntry entry, Int64 userid, ATSRole role)
        {
